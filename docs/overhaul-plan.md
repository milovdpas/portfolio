...
- [x] F4 curve model settled via AskUserQuestion (couldn't view Milo's gameplay video): chosen model = "ball traces your swipe". Launch heading = start of the stroke; curl = signed angle from the launch heading to the second-half heading (measured along cumulative path length so start-holds/end-settle don't skew it), spread over the flight; ~20° dead zone keeps near-straight swipes straight. Verified against Milo's real logged checkmark gestures: straight stays straight, rightward hooks curl right (omega 0.6-0.8, visibly banana). Telemetry still on for his device testing.
